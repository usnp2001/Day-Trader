...
def main():
    ...
    for row in rows:
        symbol = row["symbol"]
        # Limit sync to DEFAULT_SYMBOLS to stay well below the 600 requests/hour API rate limit
        if symbol in DEFAULT_SYMBOLS and (symbol.endswith(".TW") or symbol.endswith(".TWO")):
            tw_stocks.append({
                "symbol": symbol,
                "name": row["name"],
